...
#Blast design is one of the most critical operations in open-pit mining. Poorly designed blasts result in oversized rock fragments, excessive ground vibration, flyrock hazards, and increased costs associated with secondary breakage and equipment wear. Conversely, well-designed blasts improve fragmentation quality, reduce explosive consumption, and enhance overall productivity. This application provides a structured, formula-driven approach to blast design, enabling users to rapidly compute key blast geometry and charge parameters based on site-specific inputs.
...
#Figure 1: The Design Inputs screen showing the main application layout, sidebar navigation, input fields for Hole Geometry and Explosive Parameters, the Run Calculation button, and the empty Calculation History panel below.
#As shown in Figure 1, the main screen is divided into the sidebar on the left and the primary content panel on the right. On initial launch, the Design Inputs section is active and the Calculation History panel at the bottom displays a prompt indicating that no calculations have been performed yet.
...
#Figure 2: The Results & Metrics screen with the Quick Reference sidebar expanded, showing the Blasting Formulae (Burden B = Kb x d, Spacing S = Ks x B, Stemming T = 0.7 x B, Charge Weight W = π/4 x d² x ρ x Lc, Powder Factor PF = W / (B x S x H)) and Constants Used (Kb = 25, Ks = 1.25).
...
#Figure 3: The Results & Metrics screen showing the output summary bar (Burden 2.25 m, Spacing 2.81 m, Stemming 1.57 m, Total Depth 9.00 m, Charge Weight 39.0 kg, Actual PF 0.684 kg/m3), the Powder Factor Match banner, the full Parameter Summary table, and the Hole Cross-Section diagram.
#8.1 Summary Metrics Bar
#Across the top of the results panel, six key calculated values are displayed prominently in large text. These are: Burden, Spacing, Stemming, Total Depth, Charge Weight, and Actual Powder Factor. The Actual PF value is shown with an upward or downward arrow and a delta value indicating whether the actual powder factor is above or below the target. In the example shown in Figure 3, the actual PF of 0.684 kg/m3 is 0.024 kg/m3 above the target of 0.660 kg/m3.
#8.2 Powder Factor Match Banner
...
#8.4 Hole Cross-Section Diagram
...
#Figure 4: The Fragmentation Prediction screen displaying the Rosin-Rammler Fragmentation Curve (cumulative passing percentage against fragment size in mm) and the Model Parameters panel showing median fragment size x50 = 380.0 mm, uniformity index n = 1.00, rock strength 45 MPa (UCS), and charge style Single Column.
...
#Figure 5: The Fragmentation screen with chart interaction icons visible (download, zoom, full-screen), and the Key Passing Sizes table listing cumulative passing percentages at standard fragment sizes from 50 mm to 800 mm.
#10. History Section
...
#Figure 6: The History section showing the Calculation History table with two entries. The table columns are Time, Diameter (mm), Bench Height (m), Burden (m), Spacing (m), Charge (kg), Charge Style, Actual PF, Target PF, and PF Status (PASS or FAIL). A Download PDF Report button and a Clear History button are also visible.
...
#The example shown in Figure 6 contains two entries. The first entry at 21:35:52 shows a PASS result where the actual PF of 0.6842 kg/m3 is within the tolerance of the target PF of 0.66 kg/m3. The second entry at 21:35:13 shows a FAIL result because the target PF was set to 1.0 kg/m3, which is significantly outside the tolerance of the actual computed value.
